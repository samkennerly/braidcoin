...
class Network:
    ...
    def __init__(
        self, nnodes, ticksize=TICKSIZE, npeers=4, target=None, topology="sphere"
    ):
        self.t = 0  # the current "time"
        self.ticksize = (
            ticksize  # the size of a "tick": self.t += self.tick at each step
        )
        self.npeers = npeers
        self.nnodes = nnodes
        ...
        latencies = None
        for node, peers in zip(
            self.nodes,
            [
                choice(list(set(range(nnodes)) - {me}), npeers, replace=False)
...
class Node:
    ...
    def reset(self, target=None):
        self.beads = [
            self.network.beads[self.network.genesis]
        ]  # A list of beads in the order received
        self.braids = [
            Braid(self.beads)
        ]  # A list of viable braids, each having a braid tip
    ...
    def setpeers(self, peers, latencies=None):
        ...
        if latencies:
            ...
            self.latencies = sample(len(peers)) * NETWORK_SIZE
    ...
    def tick(self, newtxs=[], mine=True):
        ...
        if mine:
            ...
            if self.hremaining <= 0:
                PoW = (
                    uint256(sha256(self.nodesalt + self.nonce)) * self.target
                ) // MAX_HASH
                self.nonce += 1
...
class Braid(gt.Graph):
    ...
    def __init__(self, beads=[]):
        ...
        self.vhcolors = self.new_vertex_property(
            "vector<float>"
        )  # vertex halo colorings
        self.groups = self.new_vertex_property(
            "vector<float>"
        )  # vertex group (cohort number)
    ...
    def extend(self, bead):
        ...
        if (
            not bead.parents  # No parents -- bad block
            or not all(
                [p.hash in self.vhashes for p in bead.parents]
            )  # We don't have all parents
            or bead in self.beads
        ):  # We've already seen this bead
            return False
        self.beads.append(bead)
    ...
    def rewards(self, coinbase):
        ...
        for cohort in self.cohorts():
            for c in cohort:
                siblings = (
                    cohort
                    - self.ancestors(c, cohort)
                    - self.descendants(c, cohort)
                    - {c}
                )
                bc = self.beads[int(c)]
                # Denominator (normalization) for distribution among siblings
                bc.sibling_difficulty = MAX_HASH / (
                    sum([self.beads[int(s)].difficulty for s in siblings])
                    + bc.difficulty
                )
            N = sum(
                [
                    self.beads[int(c)].difficulty
                    / self.beads[int(c)].sibling_difficulty
                    for c in cohort
                ]
            )
            for c in cohort:
    ...
    def siblings(self):
        ...
        if self.ncohorts < 0:
            for c in c.cohorts():
                pass  # force cohorts() to generate all cohorts
    ...
    def cohorts(self, initial_cohort=None, older=False, cache=False):
        ...
        *all* beads in this cohort are ancestors of the first generation of beads in the next
        ...
        parents = ancestors = {
            h: self.next_generation(h, not older) - cohort for h in head
        }
        ncohorts = 0
        if cache and not hasattr(self, "cohort_cache"):
            ...
            self.ancestor_cache = (
                {}
            )  # The ancestors of each bead, *within* their own cohort
            self.descendant_cache = (
                {}
            )  # The descendants of each bead, *within* their own cohort
        while True:
            if cache and ncohorts in self.cohort_cache:
                yield self.cohort_cache[ncohorts]
            else:
                if cache:
                    acohort = cohort.union(
                        self.next_generation(head, older)
                    )  # add the youngest beads in the ancestor cohort
                    ancestors = {
                        int(v): frozenset(map(int, self.ancestors(v, acohort)))
                        for v in acohort
                    }
                    self.ancestor_cache[ncohorts] = ancestors
                    dcohort = cohort.union(
                        self.next_generation(head, not older)
                    )  # add the oldest beads in the descendant cohort
                    descendants = {
                        int(v): frozenset(map(int, self.descendants(v, dcohort)))
                        for v in dcohort
                    }
                    self.descendant_cache[ncohorts] = descendants
                    self.cohort_cache.append(cohort)
                    self.sibling_cache[ncohorts] = {
                        v: cohort - ancestors[v] - descendants[v] - frozenset([v])
                        for v in cohort
                    }
                yield cohort
            ncohorts += 1
            ...
            while True:
                ...
                while True:  # Update ancestors: parents plus its parents' parents
                    oldancestors = {
                        v: ancestors[v] for v in gen
                    }  # loop because ancestors may have new ancestors
                    for v in gen:
                        if all(
                            [p in ancestors for p in parents[v]]
                        ):  # If we have ancestors for all parents,
                            ancestors[v] = parents[v].union(
                                *[ancestors[p] for p in parents[v]]
                            )  # update the ancestors
                    if oldancestors == {v: ancestors[v] for v in gen}:
                        break
                if all(
                    [p in ancestors]
                    for p in frozenset.union(*[parents[v] for v in gen])
                ) and all(  # we have no missing ancestors
                    [h in ancestors[v] for h in head for v in gen]
                ):  # and everyone has all head beads as ancestors
                    cohort = frozenset.intersection(
                        *[ancestors[v] for v in gen]
                    )  # We found a new cohort
                    nexthead = self.next_generation(cohort, older) - cohort
                    tail = self.next_generation(
                        nexthead, not older
                    )  # the oldest beads in the candidate cohort
                    if all(
                        [
                            n in ancestors and p in ancestors[n]
                            for n in nexthead
                            for p in tail
                        ]
                    ):
    ...
    def common_generation(self, vs, older=True):
        ...
        if older:
            ...
            (edgef, nodef, nextgen_f) = ("in_edges", "source", self.children)
    ...
    def all_generations(self, v: gt.Vertex, older, cohort=None, limit=None):
        ...
        while gen:
            ...
            if cohort:
                result = result.intersection(cohort)
        return result
    ...
    def plot(
        ...
        **kwargs
        ...
        if layout:
            pos = layout(self, **kwargs)
        else:
            pos = self.braid_layout(**kwargs)
        n = 0

        kwargs = {
            "vertex_size": self.vsizes,
            "vertex_font_size": 10,
            "nodesfirst": True,
            "vertex_text": vlabel,
            "vertex_halo": True,
        ...
        if focusbead:
            if not hasattr(self, "sibling_cache"):
                for c in self.cohorts(cache=True):
                    pass
            ...
            if cohorts:
                for c in self.cohorts():
                    ...
                    cnum += 1
            if numbervertices:
                kwargs["vertex_text"] = self.vertex_index
